...
async function fetchStatus() {
  try {
    ...
    const res = await fetch(`http://164.92.187.207:5005/api/app/content-status/granted/7?language=ar`, {
      headers: {
...
async function fetchDesicions(){
  try{
    const token = localStorage.getItem('token');
    const res= await fetch('http://164.92.187.207:5005/api/app/category/by-filters?Language=ar&contentTypeId=7&maxResultCount=1000',{
      headers:{
        authorization: `Bearer ${token}`
      }
    });
